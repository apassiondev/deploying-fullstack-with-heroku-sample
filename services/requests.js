...
const getAllActivities = (req, res) => {
  ...
  pool
    .query(getString) // send query to select all rows from the 'my_activities' table
    .then((activityResults) => {
      let activities = activityResults.rows;
      pool
        .query(countString) // send query to get total row count from the 'my_activities' table
        .then((countResult) => {
...
const addActivityToDB = (req, res) => {
  ...
  const addString =
    "INSERT INTO my_activities (activity) VALUES ($1) RETURNING *"; // insert value into my_activities' table
...
const deleteAllActivites = (req, res) => {
  ...
  pool
    .query(removeString) // send query delete all items in the 'my_activities' table
